...
@app.route('/login')
def login():
    return redirect("https://www.facebook.com/v3.2/dialog/oauth?client_id={{   }}&redirect_uri=" + redirect_url + "&state={\"{{ st_key }}\"}")
...
def get_name(token):
    ...
    userid = ver.json()
    name = requests.get(r'https://graph.facebook.com/v3.2/' + userid['data']['user_id'] + '?access_token=' + tok)
